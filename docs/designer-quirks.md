...
Every frame on the Ampio bus is addressed by a module's MAC. Designer's MAC override field lets two devices be saved on one address, and nothing in Designer objects. This integration cannot tell those modules apart, so it leaves them out of its module list. Their objects still work, because an object carries its own address. What goes is everything that needs the module's own row. The Identify button, the buzzer, Unlock touch, and the panel lights answer a press with an error saying that no unique module row matches, and the supply voltage and temperature sensors read unknown. Whether the buzzer and the panel lights exist at all depends on whether the module answered the record sweep when the integration last started, so they can be missing too.
