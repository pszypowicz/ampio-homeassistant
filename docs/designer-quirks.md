...
An entity id is built once, from the area name, the device name and the entity name, when Home Assistant first registers the entity. After that it holds still until you rebuild it yourself from the entity's settings page. Rename a device, move it to another area, or switch the account tier, and every id stays where it is.
