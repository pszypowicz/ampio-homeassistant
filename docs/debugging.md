...
- `snapshot`: the library's health report.
  - `available`, `auth_failure`: whether the broker connection is up, and the broker's reason code when the connection loop stopped for auth.
  ...
  - `not_configured`: the Designer object numbers of the rows the integration left out. These are the objects that point at no module output, and they have no entities at all. The list carries no Designer name, and the repair on the Settings page names the same ids and the fix for them.
  ...
  - `last_payloads`: one entry per server endpoint. Each entry holds the shape of the endpoint's last reply rather than the reply itself. A table reply reads as a row count, and a reply whose JSON or table envelope is malformed reads as `**REDACTED**`. The row count is taken before the library parses the rows, so a table can show a row count while the library refused the rows themselves. The `info` entry is the server-info reply cut down to the server MAC, the account class, and the three version fields, each as the library parsed it. A version field in an unexpected form reads `**REDACTED**`, and a reply the library could not parse reads `**REDACTED**` as a whole, so the LAN address, the hardware id, and any other field the reply carries stay out of the file. A table reply is summarized because it carries device names, room names, and the account's street address.
...
- Setup degradation: a failed room-map fetch logs a warning with its consequence. A failed scene-catalogue fetch leaves the scenes out, and Home Assistant logs that the scene platform is not ready and retries it. A failed Designer-description sweep stops setup instead, and Home Assistant retries it, as described under [Setup does not finish, and stays on "Retrying setup"](faq.md#setup-does-not-finish-and-stays-on-retrying-setup). The sweep that runs when a new module device appears does not stop anything when it fails or when a new module does not answer it. A new module that has not answered any sweep since the integration started has no capability data, so a warning names it, and a reload sweeps again to build the rest of its controls.
