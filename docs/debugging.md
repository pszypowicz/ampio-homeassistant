...
- `snapshot`: the library's health report.
  ...
  - `last_payloads`: one entry per server endpoint, holding the shape of its last reply rather than the reply. A table reply reads as a row count, and a reply the library could not parse reads as `**REDACTED**`. The `info` entry keeps the server's own version and MAC fields and masks the rest. The library summarizes this way because a reply carries device names, room names, and the account's street address, and a key-based redactor cannot reach inside one string.
...
The diagnostics download no longer carries catalogue rows. To read your own object's row, subscribe to the M-SERV directly with an MQTT client and request the catalogue. The administrator login answers on `ampio/fromDB/<user>/config/devicesDetails` after an empty-keyword request, and a standard account answers on `ampio/fromDB/<user>/data/devices`.
