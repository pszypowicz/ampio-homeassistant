...
Privacy note before sharing a download publicly: credentials, host, and LAN address are redacted, and server replies are summarized rather than copied. The download itself carries no object, room, or module name, because `designer_config` lists ids, capability names, panel colors and timings, cover ids, and travel numbers, and `snapshot.modules` carries no name field either. Names surface only in the entity names your own screenshots show.
