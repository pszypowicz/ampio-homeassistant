...
- Setup degradation: a failed room-map fetch logs a warning with its consequence. A failed scene-catalogue fetch leaves the scenes out, and Home Assistant logs that the scene platform is not ready and retries it. A failed Designer-description sweep stops setup instead, and Home Assistant retries it, as described under [Setup does not finish, and stays on "Retrying setup"](faq.md#setup-does-not-finish-and-stays-on-retrying-setup). The sweep that runs when a new module device appears does not stop anything when it fails or when a new module does not answer it. It logs a warning that names those modules, and a reload builds the rest of their controls.
